...
function func_read_file{
    ...
    while($true){
        ...
        if ( # CRLF の場合
            ( $int32arr_return_or_eof_read[0] -eq $bytearr_crlf[0] ) -And
            ( $int32arr_return_or_eof_read[1] -eq $bytearr_crlf[1] )
        ){
            $writer2.Write("CRLF`r`n")
            Write-Host "CRLF"
        
        } elseif ( # CR の場合
            ( $int32arr_return_or_eof_read[0] -eq $bytearr_crlf[0] )
        ){
            $writer2.Write("CR`r`n")
            Write-Host "CR"
        
        } elseif ( # LF の場合
            ( $int32arr_return_or_eof_read[0] -eq $bytearr_crlf[1] )
        ){
